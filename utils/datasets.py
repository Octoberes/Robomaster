...
class LoadStreams:  # multiple IP or RTSP cameras
	def __init__(self, sources='streams.txt', img_size=640):
		...
		for i, s in enumerate(sources):
			...
			#assert self.cam.isOpened(), 'Failed to open %s' % s
			...
			thread = Thread(target=self.update, args=([i, 1]), daemon=True)
	...
	def update(self, index, cap):
		...
		while True:
			...
			if cap is None:
				continue
			self.im1 = cap.get_numpy_array()
			self.imgs[0] = cv2.cvtColor(self.im1, cv2.COLOR_BGR2RGB)
